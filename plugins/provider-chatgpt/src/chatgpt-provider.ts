 ...
 *
 * Connects to existing Chrome profile with ChatGPT open.
 * Supports both chat.openai.com and chatgpt.com domains.
 */
export class ChatGPTProvider implements Provider {
  ...
  // ChatGPT UI selectors (tested against chatgpt.com as of 2025-2026)
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.page = await this.browserManager.getPage('https://chatgpt.com');
  ...
  private async waitForReady(): Promise<void> {
    ...
    for (const selector of ChatGPTProvider.SELECTORS.input) {
      try {
    ...
    throw new Error('Could not find chat input. Please make sure ChatGPT is open and loaded.');
  }
  ...
  private async checkAuthorization(): Promise<void> {
    ...
    for (const selector of ChatGPTProvider.SELECTORS.loginButton) {
      try {
        const loginButton = await this.page.$(selector);
        if (loginButton) {
          throw new Error('User not authorized. Please log in to ChatGPT first.');
        }
      } catch (error) {
  ...
  private async clickSend(): Promise<void> {
    ...
    for (const selector of ChatGPTProvider.SELECTORS.sendButton) {
      try {
        const element = await this.page.$(selector);
        if (element) {
  ...
  private async isLoading(): Promise<boolean> {
    ...
    for (const selector of ChatGPTProvider.SELECTORS.loading) {
      try {
        const element = await this.page.$(selector);
        if (element) {
          return true;
  ...
  private async extractResponse(): Promise<string> {
    ...
    for (const selector of ChatGPTProvider.SELECTORS.response) {
      try {
    ...
    const mainContent = await this.page.$('main, [role="main"], .chat-container, #__next');
    if (mainContent) {
  ...
  getCapabilities(): ProviderCapabilities {
    return PROVIDER_CAPABILITIES.chatgpt;
  }
  ...
  cancel(): void {
    if (this.page) {
      for (const selector of ChatGPTProvider.SELECTORS.stopButton) {
