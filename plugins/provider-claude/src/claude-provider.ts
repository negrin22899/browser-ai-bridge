...
/**
 * Claude Provider - real implementation using Playwright
 *
 * Connects to existing Chrome profile with Claude.ai open.
 * Handles Claude's unique UI elements including contenteditable divs.
 */
export class ClaudeProvider implements Provider {
  ...
  // Claude UI selectors (tested against claude.ai as of 2025-2026)
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.page = await this.browserManager.getPage('https://claude.ai');
  ...
  private async waitForReady(): Promise<void> {
    ...
    for (const selector of ClaudeProvider.SELECTORS.input) {
      try {
    ...
    throw new Error('Could not find chat input. Please make sure Claude.ai is open and loaded.');
  }
  ...
  private async checkAuthorization(): Promise<void> {
    ...
    for (const selector of ClaudeProvider.SELECTORS.loginButton) {
      try {
        const loginButton = await this.page.$(selector);
        if (loginButton) {
          throw new Error('User not authorized. Please log in to Claude.ai first.');
        }
      } catch (error) {
    ...
    for (const selector of ClaudeProvider.SELECTORS.input) {
      try {
        ...
        if (element) {
          ...
          // Handle contenteditable divs (Claude uses ProseMirror)
          ...
          if (tagName === 'div' || isContentEditable) {
            // For contenteditable, we need to type character by character
  ...
  private async clickSend(): Promise<void> {
    ...
    for (const selector of ClaudeProvider.SELECTORS.sendButton) {
      try {
        const element = await this.page.$(selector);
        if (element) {
  ...
  private async isLoading(): Promise<boolean> {
    ...
    for (const selector of ClaudeProvider.SELECTORS.loading) {
      try {
        const element = await this.page.$(selector);
        if (element) {
          return true;
  ...
  private async extractResponse(): Promise<string> {
    ...
    for (const selector of ClaudeProvider.SELECTORS.response) {
      try {
  ...
  getCapabilities(): ProviderCapabilities {
    return PROVIDER_CAPABILITIES.claude;
  }
  ...
  cancel(): void {
    if (this.page) {
      for (const selector of ClaudeProvider.SELECTORS.stopButton) {
